...
def PlotBinaryPatterns(Texture,nPatternRow,nPatternColumn):
    ...
    for i in range(1,nPattern):
        ...
        print(np.where(Texture*nPattern<i*np.size(Texture),1.0,0.0))
...
def getDitherImg(Texture, img):
    TH, TW = Texture.shape
    H, W = img.shape
    for i in range(H):
        for j in range(W):
            if img[i][j]*TH*TW >= Texture[i%TH,j%TW]  * 255:
                img[i][j] = 255
            else:
                img[i][j] = 0
...
if(__name__=="__main__"):
    ...
    # img = np.array(Image.open('1.png'))[:,:,0]
    # img = cv2.resize(img, dsize=(160, 160), interpolation=cv2.INTER_CUBIC)
    img = cv2.imread('1024_1024_gray.png', 0)
    # img = cv2.resize(img,(256,256))
    before = img.copy()
    im = img.copy()
    Texture=GetVoidAndClusterBlueNoise((128, 128),1.9);
    getDitherImg(Texture, img)
    Texture=GetVoidAndClusterBlueNoise((128, 128),1);
    getDitherImg(Texture, im)
    print(img)
    Figure=pyplot.figure();
    Figure.add_subplot(1,2,1,xticks=[],yticks=[]);
    pyplot.imshow(img,cmap="gray",interpolation="nearest");
    Figure.add_subplot(1,2,2,xticks=[],yticks=[]);
    pyplot.imshow(im,cmap="gray",interpolation="nearest");
    pyplot.show();
